...
$(document).ready(function(){$(".ivy-export-csv").exportable({data_table:"#ivy tbody",filename:"timesheet.csv"});$(".ivy-export-json").exportable({data_table:"#ivy tbody",filename:"timesheet.json"});var c=$("#ivy tbody").ivy({preferences:{format_time:"hh:mm A",format_date:"YYYY-MM-DD",format_prec:2,insert_rows:[{began:"7:45",ended:"9:30"},{began:"9:30",ended:"10:00"},{began:"10:00",ended:"3:45p"}],weekends:!1},init:function(){var b=this,a=this.ivy,c=this.preferences,d=a.settings.classCellReadOnly;
a=$(a.getTableBodyElement());var e="<tr>";a.prev("thead").find("tr:first th").each(function(a){e+="<td";[0,3,4].includes(a)?(e+=' class="'+d+'">',0===a&&(a=moment().startOf("month").subtract(1,"day"),a=b.getNextWorkDay(a),e+=a.format(c.format_date))):e+=">";e+="</td>"});e+="</tr>";a.append(e)},getNextWorkDay:function(b){var a=this.preferences;b=b.clone();do b.add(1,"day");while(!a.weekends&&b.toDate().isWeekend());return b},refreshCells:function(){for(var b=this.ivy.getMaxRows(),a=0;a<=b;a++)this.onCellValueChangeAfter(a,
